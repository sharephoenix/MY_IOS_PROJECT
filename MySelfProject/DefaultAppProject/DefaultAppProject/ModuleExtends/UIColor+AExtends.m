...
#import "UIColor+AExtends.h"

@implementation UIColor (AExtends)
+ (UIColor *)colorWithHexAlphaString:(NSString *)hexAlphaString
{
    assert(9 == hexAlphaString.length);

    NSString *colorHex   = [NSString stringWithFormat:@"%@", [hexAlphaString substringWithRange:NSMakeRange(0, 7)]];
    NSString *alphaHex   = [NSString stringWithFormat:@"0x%@", [hexAlphaString substringWithRange:NSMakeRange(7, 2)]];
    unsigned alphaInt = [[self class] hexValueToUnsigned:alphaHex];

    return [[self class] colorWithHexString:colorHex alpha:alphaInt/255.0];
}
